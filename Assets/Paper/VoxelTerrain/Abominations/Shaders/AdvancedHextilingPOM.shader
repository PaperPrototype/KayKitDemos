Shader "Custom/AdvancedHextilingPOM"
...
Pass "Standard"
{
    ...
    GLSLPROGRAM
        ...
        Fragment
        {
            ...
            // UV offset hash: maps cell vertex → random [0,1)^2 offset.
            vec2 sHash(vec2 p)
            {
            ...
            // Rotation hash: separate constants to avoid correlation with sHash.
            float rHash(vec2 p)
            {
                return fract(sin(dot(p, vec2(41.5, 93.7))) * 43758.5453123);
            }

            // CCW rotation of uv by angle a.
            ...
            // Hexagonal grid setup via triangular lattice.
            //
            // The vertices of the triangular tessellation ARE the centers of the
            // hexagonal tessellation (they are dual grids). For any UV, exactly 3
            // hex centers enclose it; their barycentric coordinates are the blend
            // weights. Each center gets a random UV translation (ox) and one of 6
            // discrete orientations (r) at multiples of 60°, exploiting the hex
            // grid's native 6-fold rotational symmetry.
                          ...
                          out vec3 bary)
            {
                ...
                const float PI_OVER_3 = 1.04719755; // π/3
                ...
                // floor(hash * 6) * π/3 maps uniformly to {0, 60, 120, 180, 240, 300} degrees.
                r1 = floor(rHash(v1) * 6.0) * PI_OVER_3;
                r2 = floor(rHash(v2) * 6.0) * PI_OVER_3;
                r3 = floor(rHash(v3) * 6.0) * PI_OVER_3;
            }

            // Hex-tiling sample: rotate+offset UV per cell, variance-preserving blend.
            ...
            // Hex-tiling normal map sample with rotation correction.
            //
            // When we rotate the UV by r, the effective texture tangent axes rotate by
            // the same angle. To express the decoded normal back in the unrotated axis
            // tangent frame — so the per-axis world-space transform works correctly —
            // we rotate the normal's XY by +r (the same angle as the UV rotation).
            //
            // Derivation: sampling at hexRot(uv,r) maps surface tangent T' =
            //   cos(r)*T_orig + sin(r)*B_orig, so (nx,ny) in the rotated frame becomes
            //   (nx*cos(r) - ny*sin(r), nx*sin(r) + ny*cos(r)) in the original frame.
            ...
            void main()
            {
                ...
                // Rotation is intentionally omitted here: applying different rotations
                // per cell during the march would make adjacent cells converge to
                // inconsistent depths. The translation offset alone breaks height-field
                // tiling while keeping the march stable. Full rotation is applied only
                // to the final texture reads where it is visually critical.
                ...
                // -----------------------------------------------------------------------
                // Seamless world-space POM with triplanar height and hex depth offsets.
                // View direction decomposed into depth rate (viewDotN) and surface-tangent
                // drift — both smooth functions of N, no axis branches, no seam.
                ...
                // Height-lerp triplanar blend: hex-sampled heights bias weights so
                // taller features dominate at axis transitions.
                vec3 fw = weights;
                if (_HeightBlendStrength > 0.001)
                {
                    float hX = hexSample(_ParallaxMap, uvX, ox1X, ox2X, ox3X, r1X, r2X, r3X, baryX).g;
                    float hY = hexSample(_ParallaxMap, uvY, ox1Y, ox2Y, ox3Y, r1Y, r2Y, r3Y, baryY).g;
                    float hZ = hexSample(_ParallaxMap, uvZ, ox1Z, ox2Z, ox3Z, r1Z, r2Z, r3Z, baryZ).g;
                ...
                vec4 albedo = hexSample(_MainTex, uvX, ox1X, ox2X, ox3X, r1X, r2X, r3X, baryX) * fw.x
                            + hexSample(_MainTex, uvY, ox1Y, ox2Y, ox3Y, r1Y, r2Y, r3Y, baryY) * fw.y
                            + hexSample(_MainTex, uvZ, ox1Z, ox2Z, ox3Z, r1Z, r2Z, r3Z, baryZ) * fw.z;
                ...
                // hexSampleNormal returns the normal in the unrotated axis tangent frame, so
                // the standard per-axis transform applies unchanged.
                //   X: T=(0,0,1) B=(0,1,0)  → world (tn.z*sign(N.x), tn.y, tn.x)
                //   Y: T=(1,0,0) B=(0,0,1)  → world (tn.x, tn.z*sign(N.y), tn.y)
                //   Z: T=(1,0,0) B=(0,1,0)  → world (tn.x, tn.y, tn.z*sign(N.z))
                ...
                vec4 surface  = hexSample(_SurfaceTex, uvX, ox1X, ox2X, ox3X, r1X, r2X, r3X, baryX) * fw.x
                              + hexSample(_SurfaceTex, uvY, ox1Y, ox2Y, ox3Y, r1Y, r2Y, r3Y, baryY) * fw.y
                              + hexSample(_SurfaceTex, uvZ, ox1Z, ox2Z, ox3Z, r1Z, r2Z, r3Z, baryZ) * fw.z;
                float ao        = 1.0 - surface.r;
                float roughness = surface.g;
                float metallic  = surface.b;

                // Translucency map: G=extra occlusion, B=thickness
                vec4 transOcc = hexSample(_TranslucencyMap, uvX, ox1X, ox2X, ox3X, r1X, r2X, r3X, baryX) * fw.x
                              + hexSample(_TranslucencyMap, uvY, ox1Y, ox2Y, ox3Y, r1Y, r2Y, r3Y, baryY) * fw.y
                              + hexSample(_TranslucencyMap, uvZ, ox1Z, ox2Z, ox3Z, r1Z, r2Z, r3Z, baryZ) * fw.z;
                ao *= transOcc.g;
                float translucency = transOcc.b * _TranslucencyStrength;

                // Emission
                vec3 emission = (hexSample(_EmissionTex, uvX, ox1X, ox2X, ox3X, r1X, r2X, r3X, baryX).rgb * fw.x
                               + hexSample(_EmissionTex, uvY, ox1Y, ox2Y, ox3Y, r1Y, r2Y, r3Y, baryY).rgb * fw.y
                               + hexSample(_EmissionTex, uvZ, ox1Z, ox2Z, ox3Z, r1Z, r2Z, r3Z, baryZ).rgb * fw.z)
                              * _EmissionIntensity;
        ...
        Fragment
        {
            ...
            void main()
            {
                if (_AlphaCutoff > 0.0)
                {
                    ...
                    // Simple (non-variance-preserving) alpha blend — sufficient for discard
